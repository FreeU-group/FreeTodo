...
ROOT_HELP_TEXT = {
    "en": (
        ...
        "Quick start:\n"
        "  1. Start server: uv run --directory local-api python server.py\n"
        "  2. List todos:   uv run --directory local-api freetodo todo list --json\n"
        "  3. Create todo:  uv run --directory local-api freetodo todo create --input todo.json --json\n\n"
    ...
    "zh": (
        ...
        "快速开始：\n"
        "  1. 启动后端：uv run --directory local-api python server.py\n"
        "  2. 查看待办：uv run --directory local-api freetodo todo list --json\n"
        "  3. 创建待办：uv run --directory local-api freetodo todo create --input todo.json --json\n\n"
        "环境变量：\n"
