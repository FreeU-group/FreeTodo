...
class SchedulerManager:
    ...
    def _cleanup_stale_jobs(self):
        """清理持久化存储中的过期任务，防止反序列化失败

        所有任务会在 JobManager.start_all() 中重新注册，
        因此启动前清空持久化存储是安全的。
        """
        if not self.scheduler:
            return

        jobstore = self.scheduler._jobstores.get("default")
        if not jobstore or not hasattr(jobstore, "engine"):
            return
        ...
        try:
            ...
            with jobstore.engine.connect() as conn:
                ...
                if result.rowcount > 0:
                    logger.info(f"已清理 {result.rowcount} 个持久化的过期任务")
                conn.commit()
