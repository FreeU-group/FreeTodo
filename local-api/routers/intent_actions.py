...
def _create_todo_from_action(action_id: str) -> tuple[str, int | None]:
    ...
    try:
        ...
        name = str(todo_data.get("name") or action.title).strip() or action.title
        ...
        return name, getattr(result, "id", None)
...
async def confirm_todo(action_id: str) -> ActionResponse:
    """User confirms creating a todo from a pending action."""
    ...
    try:
        name, todo_id = _create_todo_from_action(action_id)
...
@router.post("/{action_id}/confirm-and-execute")
async def confirm_and_execute(
    action_id: str,
    chat_service: ChatService = Depends(get_chat_service),
) -> ActionResponse:
    """Confirm the action as a todo and immediately continue into execution chat."""
    logger.info("[FLOW][ConfirmExecute] 收到确认并执行请求: action_id=%s", action_id)
    action = get_action(action_id)
    if action is None:
        logger.warning("[FLOW][ConfirmExecute] action不存在: action_id=%s → 404", action_id)
        raise HTTPException(status_code=404, detail="Action not found")
    if action.status != ActionStatus.PENDING:
        logger.warning(
            "[FLOW][ConfirmExecute] 状态不对: expected=PENDING, got=%s → 409",
            action.status.value,
        )
        payload = ActionResponse(
            success=False,
    ...
    name, todo_id = _create_todo_from_action(action_id)
    logger.info(
        "[FLOW][ConfirmExecute] ✓ 待办已确认: action_id=%s, todo_id=%s, name=%s",
        action_id,
        todo_id,
        name,
    )
    response = _build_execution_response(action_id, chat_service=chat_service)
    response.message = f"已确认并开始执行：{name}"
    response.data = {
        **(response.data or {}),
        "todo_id": todo_id,
    }
...
async def execute_task(
    ...
    return _build_execution_response(action_id, chat_service=chat_service)
