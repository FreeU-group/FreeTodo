...
export default function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
	...
	const [initialShowProjectManagement, setInitialShowProjectManagement] =
		useState(false); // 记录初始值
	...
	const loadConfig = useCallback(async () => {
		...
		try {
			...
			if (response.data.success) {
				...
				const blacklistAppsArray = Array.isArray(apps)
					? apps
					: typeof apps === "string"
						? apps
								.split(",")
				...
				const newSettings = {
					...
					jobsRecorderParamsBlacklistEnabled:
						config.jobsRecorderParamsBlacklistEnabled ?? false,
	...
	const handleTest = async () => {
		...
		try {
			...
			setMessage({
				type: "error",
				text: `${t.settings.testFailed}: ${errorMsg}`,
			});
		} finally {
			setTesting(false);
		}
		...
		try {
			...
			if (hasLlmConfig) {
				...
				await api.saveConfig({
					...
					jobsRecorderParamsBlacklistApps:
						settings.jobsRecorderParamsBlacklistApps,
			...
			if (response.data.success) {
				...
				if (schedulerChanged) {
					...
					window.dispatchEvent(
						new CustomEvent("schedulerVisibilityChange", {
							detail: {
				...
				// 保存费用统计显示设置
				const costTrackingChanged =
					showCostTracking !== initialShowCostTracking;
				if (costTrackingChanged) {
					...
					const currentPath = window.location.pathname;
					window.dispatchEvent(
						new CustomEvent("costTrackingVisibilityChange", {
							detail: {
				...
				// 保存项目管理显示设置
				const projectManagementChanged =
					showProjectManagement !== initialShowProjectManagement;
				if (projectManagementChanged) {
					localStorage.setItem(
						"showProjectManagement",
						String(showProjectManagement),
					);
					...
					const currentPath = window.location.pathname;
					window.dispatchEvent(
						new CustomEvent("projectManagementVisibilityChange", {
							detail: {
	...
	const handleAddBlacklistApp = (app: string) => {
		...
		if (
			trimmedApp &&
			!settings.jobsRecorderParamsBlacklistApps.includes(trimmedApp)
		) {
			setSettings((prev) => ({
				...prev,
				jobsRecorderParamsBlacklistApps: [
					...prev.jobsRecorderParamsBlacklistApps,
					trimmedApp,
				],
			}));
		}
	...
	const handleRemoveBlacklistApp = (app: string) => {
		setSettings((prev) => ({
			...prev,
			jobsRecorderParamsBlacklistApps:
				prev.jobsRecorderParamsBlacklistApps.filter((a) => a !== app),
		}));
	};
	...
	const handleBlacklistKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
		if (e.key === "Enter" && blacklistInput.trim()) {
			...
			e.key === "Backspace" &&
			!blacklistInput &&
			settings.jobsRecorderParamsBlacklistApps.length > 0
		) {
			// 如果输入框为空且按下 Backspace，删除最后一个标签
			const lastApp =
				settings.jobsRecorderParamsBlacklistApps[
					settings.jobsRecorderParamsBlacklistApps.length - 1
				];
									...
									<div className="space-y-3">
										...
										<div>
											...
											<Input
												...
												onChange={(e) =>
													handleChange("llmBaseUrl", e.target.value)
												}
											/>
										</div>
										...
										<div className="pt-1">
											<Button
												...
												onClick={handleTest}
												disabled={
													testing || !settings.llmApiKey || !settings.llmBaseUrl
												}
												...
												{settings.jobsRecorderParamsBlacklistEnabled && (
													<div>
														...
														<div className="border border-input rounded-md px-2 py-1.5 min-h-[38px] flex flex-wrap gap-1.5 items-center bg-background focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
															{settings.jobsRecorderParamsBlacklistApps.map(
																(app) => (
																	<span
																		key={app}
										...
										<div className="space-y-4">
											...
											{settings.chatEnableHistory && (
												<div>
													...
													/>
													<p className="text-xs text-muted-foreground mt-1">
														{t.settings.contextRoundsDesc.replace(
															"{rounds}",
										...
										{settings.chatEnableHistory && (
											<div>
												...
												<p className="text-xs text-muted-foreground mt-1">
													{t.settings.contextRoundsDesc.replace(
														"{rounds}",
														settings.chatHistoryLimit.toString(),
													)}
												</p>
											</div>
										)}
									...
									<div className="flex items-center justify-between">
										...
										<label className="relative inline-flex items-center cursor-pointer">
											<input
												...
												onChange={(e) =>
													handleCostTrackingToggle(e.target.checked)
												}
											/>
											<div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary/20 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
										</label>
									...
									<div className="flex items-center justify-between">
										...
										<label className="relative inline-flex items-center cursor-pointer">
											<input
												...
												onChange={(e) =>
													handleProjectManagementToggle(e.target.checked)
												}
											/>
											<div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary/20 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
										</label>
