...
export function formatRelativeTime(
	...
	if (diff < 3600)
		return timeTranslations.minutesAgo.replace(
			"{count}",
			Math.floor(diff / 60).toString(),
		);
	if (diff < 86400)
		return timeTranslations.hoursAgo.replace(
			"{count}",
			Math.floor(diff / 3600).toString(),
		);
	if (diff < 604800)
		return timeTranslations.daysAgo.replace(
			"{count}",
			Math.floor(diff / 86400).toString(),
		);
