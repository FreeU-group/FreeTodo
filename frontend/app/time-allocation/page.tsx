...
const getReplaceIcon = (appName: string): string => {
	...
	if (index === undefined) {
		return `/app-icons-replace/${REPLACE_ICONS[0]}`;
	}
...
export default function TimeAllocationPage() {
	...
	const loadData = async (s = startDate, e = endDate) => {
		setLoading(true);
		try {
			const response = await api.getTimeAllocation({
				start_date: s,
				end_date: e,
			});
	...
	const getMaxHourlyUsage = (): number => {
		if (!data) return 0;
		return Math.max(
			...data.daily_distribution.map((h) => {
				const totalSeconds = Object.values(h.apps).reduce(
					(sum, time) => sum + time,
					0,
				);
		...
		<div className="container mx-auto px-4 py-8">
			...
			{data && (
				<>
						...
						<CardContent>
							<ChartContainer config={chartConfig} className="h-[400px] w-full">
								...
								>
									...
									<XAxis
										...
										tickFormatter={(value) =>
											value % 4 === 0
												? t.timeAllocation.hourLabel.replace(
														"{hour}",
														value.toString(),
													)
												: ""
									...
									/>
									<Bar
										dataKey="file_management"
										stackId="a"
										fill="var(--color-file_management)"
										radius={[0, 0, 0, 0]}
									/>
									<Bar
										dataKey="office"
							...
							<div className="flex flex-wrap gap-4 pt-4 border-t mt-4">
								{(
									...
									const categoryTime = apps.reduce(
										(sum, app) => sum + app.total_time,
										0,
									);
									const bgColor =
										chartConfig[category]?.color || "var(--chart-6)";
									return (
					...
					{selectedApp &&
						(() => {
							const selectedAppData = data.app_details.find(
								(app) => app.app_name === selectedApp,
							);
							const appCategory = selectedAppData?.category || "other";
							const appColor =
								chartConfig[appCategory as keyof typeof chartConfig]?.color ||
								"var(--chart-6)";
										...
										>
											<BarChart
												data={Array.from({ length: 24 }, (_, i) => ({
													hour: i,
													hourLabel: t.timeAllocation.hourLabel.replace(
														"{hour}",
														i.toString(),
													),
											...
											>
												...
												<YAxis
													...
													domain={[
														0,
														getNiceMaxValue(getAppMaxUsage(selectedApp)),
													]}
												/>
												<ChartTooltip
													content={
							...
							{data.app_details.length === 0 ? (
								...
								<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
									{data.app_details.map((app, index) => {
										...
										const categoryColor =
											chartConfig[appCategory as keyof typeof chartConfig]
												?.color || "var(--chart-6)";
														...
														<img
															...
															onError={(e) => {
																try {
																	...
																	const replaceIconPath = getReplaceIcon(
																		app.app_name,
																	);
																	if (target.src !== replaceIconPath) {
																		...
																		if (
																			...
																			fallback.textContent = getAppDisplayName(
																				app.app_name,
																			)
																				.charAt(0)
																				.toUpperCase();
