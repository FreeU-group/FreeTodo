...
import type { Context, Project, Task, TaskProgress } from "@/lib/types";
...
export default function TaskDetailPage() {
	...
	const taskId = parseInt((params?.taskId as string) || "0", 10);
	...
	const [latestProgress, setLatestProgress] = useState<TaskProgress | null>(
		null,
	);
		...
		try {
			...
			const errorMsg =
				(error as { response?: { data?: { detail?: string } } }).response?.data
					?.detail || t.taskDetail.progressGenerateFailed;
	...
	const formatDate = (dateString?: string) => {
		...
		try {
			const format =
				locale === "zh" ? "YYYY年MM月DD日 HH:mm" : "YYYY-MM-DD HH:mm";
				...
				{activeTab === "info" ? (
									...
									<div>
										<div className="flex items-center justify-between mb-3 text-xs text-muted-foreground">
											<span>
												...
												{formatDate(latestProgress.generated_at)}
											</span>
											<span>
												{t.taskDetail.basedOnContexts.replace(
													"{count}",
													latestProgress.context_count.toString(),
												)}
											</span>
										</div>
					...
					<div className="space-y-6">
						...
						<Card>
							<CardHeader>
								<CardTitle>
									{t.taskDetail.linkedContexts} ({contexts.length})
								</CardTitle>
							</CardHeader>
							<CardContent>
								{contexts.length === 0 ? (
						...
						<Card>
							<CardHeader>
								<CardTitle>
									{t.taskDetail.availableContexts} (
									{unassociatedContexts.length})
								</CardTitle>
							</CardHeader>
							<CardContent>
