...
def main() -> None:
    ...
    parser.add_argument("--port", type=int, default=9876, help="本地 API 端口（需 fastapi）")
    ...
    print()
    print("  通知源:")
    print(f"    [1] 推送通知  /api/sensor/notifications  (每 {SENSOR_NOTIFY_POLL_INTERVAL}s)")
    print(f"    [2] 通用通知  /api/notifications          (每 {GENERAL_NOTIFY_POLL_INTERVAL}s)")
    print(f"    [3] 待办草稿  /api/todos?status=draft      (每 {DRAFT_TODO_POLL_INTERVAL}s)")
    print()
    ...
    threading.Thread(
        target=_poll_draft_todos, args=(http_client,), daemon=True
    ).start()
