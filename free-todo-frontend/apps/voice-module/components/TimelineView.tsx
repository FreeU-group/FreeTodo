...
export function TimelineView({
  ...
  // 过滤非临时片段
  const validSegments = segments.filter(s => !s.isInterim && s.rawText && s.rawText.trim().length > 0);
  ...
  const isNearCurrentTime = (segment: TranscriptSegment): boolean => {
    ...
    const segmentEnd = segment.audioEnd ? segment.audioEnd / 1000 : segmentTime + 5;
      ...
      <div className="relative">
        ...
        <div className="space-y-4">
          {validSegments.map((segment, index) => {
            const segmentTime = getSegmentTime(segment);
            const isHighlighted = highlightedSegmentId === segment.id || isNearCurrentTime(segment);
