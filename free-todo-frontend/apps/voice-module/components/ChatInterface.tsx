		...
		<div className="flex flex-col h-full bg-card/50 border-l border-border">
			<div className="p-4 border-b border-border bg-card/80 backdrop-blur">
				...
				<p className="text-xs text-muted-foreground mt-1">
					可以随时提问。如果有录音内容，我会基于最近 10
					分钟的转录内容回答；如果没有录音内容，我也可以回答一般性问题。
				</p>
			</div>
			...
			<div className="flex-1 overflow-y-auto p-4 space-y-4">
				{messages.map((msg, index) => {
					...
					const isStreaming =
						isLoading && isLastMessage && msg.role === "model";
								...
								{msg.role === "model" ? (
									<div className="prose prose-sm max-w-none dark:prose-invert prose-p:my-1.5 prose-p:leading-relaxed prose-ul:my-1.5 prose-ol:my-1.5 prose-li:my-0.5">
										<ReactMarkdown
											...
											components={{
												h1: ({ children }) => (
													<h1 className="text-base font-bold mb-2 mt-0">
														{children}
													</h1>
												),
												h2: ({ children }) => (
												...
												pre: ({ children }) => (
													<pre className="bg-muted/50 p-2 rounded text-xs font-mono overflow-x-auto my-2">
														{children}
													</pre>
												),
												blockquote: ({ children }) => (
