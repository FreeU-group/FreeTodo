		...
		try {
					...
					if (data.text && this.onResult) {
						...
						if (
							...
							if (
								...
								console.warn(
									`[WebSocketRecognitionService] 后端时间戳格式错误: startTime=${data.startTime}, endTime=${data.endTime}，使用前端估算`,
								);
							}
						}
			...
			this.ws.onclose = (event) => {
				...
				if (
					...
					console.log(
						`[WebSocketRecognitionService] 尝试重连 (${this.reconnectAttempts}/${this.maxReconnectAttempts})，${delay}ms 后重试...`,
					);
			...
			const err =
				error instanceof Error ? error : new Error("无法连接 WebSocket");
			if (this.onError) {
				this.onError(err);
			}
			if (this.onStatusChange) {
