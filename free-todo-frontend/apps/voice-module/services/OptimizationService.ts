...
export class OptimizationService {
  ...
  private initializeAIClient(): void {
    ...
    if (apiKey) {
      ...
      if (typeof window !== 'undefined' && window.location) {
        ...
        baseURL = origin && origin !== 'null' && origin !== 'undefined'
          ? `${origin}/api/deepseek`
      ...
      console.warn('[OptimizationService] Missing NEXT_PUBLIC_DEEPSEEK_API_KEY');
    }
  }
  ...
  private async optimizeSegment(segment: TranscriptSegment): Promise<void> {
    if (!this.aiClient) {
      console.warn('[OptimizationService] AI client not initialized, skipping optimization');
      if (this.onOptimized) {
    ...
    try {
      ...
      const optimizedText = response.choices?.[0]?.message?.content?.trim() || segment.rawText;
      ...
      console.error(`[OptimizationService] Optimization failed for segment ${segment.id}:`, error);
