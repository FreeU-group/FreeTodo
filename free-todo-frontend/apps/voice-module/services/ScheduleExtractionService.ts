...
export class ScheduleExtractionService {
	...
	private async extractSchedules(segment: TranscriptSegment): Promise<void> {
		...
		try {
			console.log(
				"[ScheduleExtraction] 🤖 开始调用LLM API提取日程，片段ID:",
				segment.id,
				"文本长度:",
				textToUse.length,
			);
			...
			if (data.schedules && data.schedules.length > 0) {
				...
				console.log(
					`[ScheduleExtraction] ✅ LLM提取到 ${data.schedules.length} 个日程`,
				);
			} else {
				console.log(
					`[ScheduleExtraction] ℹ️ LLM未提取到日程（文本可能不包含日程信息）`,
				);
			}
		} catch (error) {
			console.error(
				`[ScheduleExtraction] ❌ 提取失败，片段ID: ${segment.id}`,
				error,
			);
			if (this.onError) {
				const err =
					error instanceof Error
						? error
						: new Error("Schedule extraction failed");
				this.onError(err);
			}
		}
	}
