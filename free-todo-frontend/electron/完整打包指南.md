...
```bash
# 在项目根目录
# 使用 uv（推荐）- 这会自动创建和配置 .venv 虚拟环境
uv sync --group dev

# 或使用 pip
pip install -r requirements.txt

# PyInstaller 包含在 dev 组中，uv sync --group dev 会自动安装
# 如果手动安装：
...
**重要**: 构建脚本现在会自动使用项目目录下的 `.venv` 虚拟环境，确保所有依赖都来自项目虚拟环境，而不是系统 Python 环境。这样可以避免依赖版本冲突和缺失模块的问题。
...
2. 验证依赖是否在 `.venv` 中：
   ```bash
   .venv/bin/python -c "import uvicorn; import fastapi; print('Dependencies OK')"
   ```
...
4. 检查 `pyinstaller.spec` 中的 `hiddenimports` 列表，添加缺失的模块
...
**常见问题**：
- 如果看到 `No module named 'uvicorn'` 等错误，说明系统 Python 环境缺少依赖，但现在构建脚本会使用 `.venv`，应该不会出现此问题。
- 如果 `.venv` 中缺少依赖，运行 `uv sync --group dev` 重新同步依赖。
...
**变更说明**：
- 后端构建脚本 (`lifetrace/scripts/build-backend.sh`) 现在默认使用项目目录下的 `.venv` 虚拟环境
- 这确保了所有 Python 依赖（包括 uvicorn、fastapi 等）都来自项目的虚拟环境，而不是系统 Python
- 避免了依赖版本冲突和模块缺失的问题

**迁移说明**：
- 如果之前使用系统 Python 构建，现在需要确保 `.venv` 存在：
  ```bash
  cd /path/to/LifeTrace
  uv sync --group dev
  ```
- 构建脚本会自动检测并使用 `.venv`，无需额外配置
