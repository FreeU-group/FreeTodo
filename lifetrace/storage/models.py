...
class SpeakerProfile(TimestampMixin, table=True):
    ...
    id: int | None = Field(default=None, primary_key=True)
    name: str = Field(max_length=100)  # 说话人名称（初始自动生成，可手动修改）
    description: str | None = Field(default=None, max_length=500)  # 说话人描述
    sample_count: int = Field(default=0)  # 已注册的声纹样本数
...
class SpeakerVoiceprint(TimestampMixin, table=True):
    ...
    id: int | None = Field(default=None, primary_key=True)
    speaker_profile_id: int = Field(index=True)  # 关联说话人档案ID
    embedding: str = Field(sa_column=Column(Text))  # base64编码的float32向量
    embedding_dim: int = Field(default=192)  # 向量维度
    audio_duration: float = Field(default=0.0)  # 音频时长（秒）
    quality_score: float = Field(default=0.0)  # 质量分数（0-1）
