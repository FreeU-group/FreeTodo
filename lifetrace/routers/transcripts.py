...
async def batch_save_transcripts(  # noqa: C901, PLR0912, PLR0915
    ...
    try:
        if not request.transcripts:
            ...
            return BatchSaveResponse(saved=0, message="没有转录文本需要保存")
        ...
        for transcript in request.transcripts:
            try:
                ...
                if not transcript.id:
                    logger.warning("转录文本缺少 id，跳过")
                    continue
                if not transcript.rawText or not transcript.rawText.strip():
                    logger.warning(f"转录文本 id={transcript.id} 的 rawText 为空，跳过")
                    continue
                if not isinstance(transcript.audioStart, int) or not isinstance(
                    transcript.audioEnd, int
                ):
                    logger.warning(
                        f"转录文本 id={transcript.id} 的 audioStart/audioEnd 不是整数，跳过"
                    )
                    continue
                if transcript.audioStart < 0 or transcript.audioEnd <= transcript.audioStart:
                    logger.warning(
                        f"转录文本 id={transcript.id} 的音频时间范围无效: {transcript.audioStart}-{transcript.audioEnd}，跳过"
                    )
                    continue
                ...
                try:
                    ...
                    logger.warning(
                        f"转录文本 id={transcript.id} 的时间戳格式无效: {transcript.timestamp}, error={e}"
                    )
                    continue
                ...
                with get_session() as session:
                    ...
                    if transcript.audioFileId:
                        ...
                        audio_recording = (
                            ...
                            .filter(AudioRecording.segment_id == transcript.audioFileId)
                            .first()
                        )
                        if audio_recording:
                    ...
                    existing = (
                        ...
                        .filter(TranscriptSegment.segment_id == transcript.id)
                        .first()
                    )
                    ...
                    if audio_recording_id:
                        audio_recording = (
                            session.query(AudioRecording)
                            .filter(AudioRecording.id == audio_recording_id)
                            .first()
                        )
                        if audio_recording:
                            # 合并所有片段的文本
                            all_segments = (
                                session.query(TranscriptSegment)
                                .filter(TranscriptSegment.audio_recording_id == audio_recording_id)
                                .order_by(TranscriptSegment.audio_start)
                                .all()
                            )
                            ...
                            if all_segments:
                                ...
                                optimized_texts = [
                                    seg.optimized_text for seg in all_segments if seg.optimized_text
                                ]
                ...
                logger.error(
                    f"保存转录文本失败: id={transcript.id if hasattr(transcript, 'id') else 'unknown'}, error={e}",
                    exc_info=True,
                )
...
async def query_transcripts(
    ...
    try:
        logger.debug(
            f"查询转录文本: startTime={startTime}, endTime={endTime}, audioFileId={audioFileId}"
        )
        ...
        with get_session() as session:
            ...
            if audioFileId:
                ...
                segments = (
                    query.filter(TranscriptSegment.audio_file_id == audioFileId)
                    .order_by(TranscriptSegment.timestamp)
                    .all()
                )
                logger.info(
                    f"根据音频ID查询转录文本: audioFileId={audioFileId}, 查询到 {len(segments)} 条"
                )
