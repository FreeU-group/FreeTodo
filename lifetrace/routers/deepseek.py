...
async def chat_completions(request: ChatCompletionRequest):
    ...
    try:
        ...
        if not llm_client.is_available():
            ...
            raise HTTPException(
                status_code=503,
                detail="LLM服务当前不可用，请检查 LLM 配置（API Key 和 Base URL）"
            )
        ...
        use_model = llm_client.model if request.model == "deepseek-chat" else (request.model or llm_client.model)
        ...
        # 调用 LLM 客户端
        logger.info(f"DeepSeek API 请求: model={completion_params['model']}, messages={len(messages)}, stream={request.stream}")
