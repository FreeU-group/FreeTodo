...
class IncrementalContext:
    ...
    def __init__(self, context_duration: float = 1.0, sample_rate: int = 16000):
        ...
        logger.debug(
            f"增量上下文初始化: context_duration={context_duration}s, max_samples={self.max_context_samples}"
        )
...
class ImprovedVAD:
    ...
    def __init__(
        ...
        if is_system_audio:
            ...
            logger.info(
                f"🎯 系统音频模式：VAD 阈值降低到 {self.threshold:.6f}（原阈值：{threshold:.6f}）"
            )
        else:
    ...
    def detect(self, audio_array: np.ndarray) -> str | None:
        """检测语音事件
    ...
    def _detect_voice(self, audio_array: np.ndarray) -> bool:
        ...
        if self.is_system_audio:
            ...
            voice_detected = (
                rms > self.threshold
                or (rms > self.threshold * 0.3 and zcr > zcr_threshold)  # 降低 RMS 要求
                or (
                    rms > self.threshold * 0.2 and spectral_energy > spectral_threshold
                )  # 进一步降低要求
            )
        else:
            # 麦克风音频：使用原有逻辑
...
class WhisperLiveKitNativeProcessor:
    ...
    def __init__(
        ...
        self.vad = ImprovedVAD(
            threshold=vad_threshold, min_silence_duration=0.5, is_system_audio=is_system_audio
        )
        ...
        if is_system_audio:
            ...
            logger.info(
                f"🎯 系统音频模式：音频质量检测阈值降低（RMS: {self.audio_quality_rms_threshold:.6f}, Max: {self.audio_quality_max_threshold:.6f}）"
            )
        else:
        ...
        logger.info(
            f"✅ WhisperLiveKit 原生处理器初始化: chunk={chunk_duration}s, overlap={overlap}s, context={context_duration}s"
        )
    ...
    async def try_process(self, model) -> dict | None:
        ...
        if should_process and not self.is_processing:
            logger.debug(
                f"🎯 触发处理: samples={current_samples}, time_since_last={time_since_last:.3f}s, event={event_triggered}, time={time_triggered}, overflow={buffer_overflow}"
            )
        ...
        try:
            ...
            if (
                audio_rms < self.audio_quality_rms_threshold
                or audio_max < self.audio_quality_max_threshold
            ):
                # ⚡ 系统音频：即使质量较低也记录日志，但不一定跳过（可能包含有效语音）
                if self.is_system_audio:
                    logger.debug(
                        f"⚠️ 系统音频质量较低（但可能包含有效语音）: rms={audio_rms:.6f}, max={audio_max:.6f}, 阈值: rms>{self.audio_quality_rms_threshold:.6f}, max>{self.audio_quality_max_threshold:.6f}"
                    )
                    ...
                    # 麦克风：使用原有逻辑
                    logger.debug(
                        f"⚠️ 音频质量太低，跳过处理: rms={audio_rms:.6f}, max={audio_max:.6f}"
                    )
                    samples_to_remove = min(process_samples // 2, len(self.pcm_buffer) // 2)
                    bytes_to_remove = samples_to_remove * 2
                    for _ in range(bytes_to_remove):
            ...
            if self.is_system_audio:
                logger.debug(
                    f"📊 系统音频数据: samples={len(audio_float)}, rms={audio_rms:.6f}, max={audio_max:.6f}, 阈值: rms>{self.audio_quality_rms_threshold:.6f}, max>{self.audio_quality_max_threshold:.6f}"
                )
            else:
                logger.debug(
                    f"📊 音频数据: samples={len(audio_float)}, rms={audio_rms:.6f}, max={audio_max:.6f}"
                )
        ...
        try:
            ...
            def transcribe_task():
                ...
                if self.is_system_audio:
                    ...
                    logger.debug(
                        "🎯 系统音频模式：使用优化的转录参数（no_speech_threshold=0.3, log_prob_threshold=-1.5）"
                    )
                else:
                    # 麦克风：使用原有参数
                    transcribe_params = {
    ...
    async def flush(self, model) -> dict | None:
        """刷新剩余数据"""
        ...
        while True:
            try:
                ...
                if websocket.client_state.name == "CONNECTED":
                    ...
                    last_ping_time = time.time()
                    logger.debug(
                        f"📤 发送 keepalive ping (等待 pong，上次 pong: {time.time() - last_pong_time:.1f}秒前)"
                    )
            ...
            try:
                ...
                if "bytes" in message:
                    ...
                    if result:
                        ...
                        logger.info(
                            f"✅ 发送识别结果: text={result.get('text', '')[:50]}..., isFinal={result.get('isFinal', False)}, startTime={result.get('startTime', 0):.2f}s"
                        )
                    ...
                    if text_msg == "pong":
                        ...
                        logger.debug(
                            f"📥 收到 keepalive pong (距离上次 ping: {time.time() - last_ping_time:.1f}秒)"
                        )
                        continue
