...
# 常量定义
DUPLICATE_SCHEDULE_TIME_THRESHOLD_SECONDS = 60  # 重复日程的时间阈值（秒）
MIN_HOUR_FOR_PM_OFFSET = 2  # 下午时间偏移的最小小时数
PM_HOUR_OFFSET = 3  # 下午时间偏移量（小时）
MAX_HOUR_FOR_TIME_PARSING = 12  # 时间解析的最大小时数
...
class AudioScheduleExtractionRequest(BaseModel):
    ...
    reference_time: datetime = Field(
        default_factory=datetime.now, description="参考时间（用于解析相对时间）"
    )
...
class AudioScheduleExtractionResponse(BaseModel):
    ...
    schedules: list[ExtractedScheduleItem] = Field(
        default_factory=list, description="提取的日程列表"
    )
...
async def _extract_schedules_with_llm(
    text: str, reference_time: datetime
) -> list[ExtractedScheduleItem]:
    """使用LLM智能提取日程信息"""
            ...
            for item in schedules_data:
                try:
                    schedule_time = datetime.fromisoformat(
                        item["schedule_time"].replace("Z", "+00:00")
                    )
                    schedules.append(
            ...
            for match in re.finditer(pattern, text):
                ...
                if schedule_time:
                    ...
                    if schedule_time and not any(
                        s.schedule_time
                        and abs((s.schedule_time - schedule_time).total_seconds())
                        < DUPLICATE_SCHEDULE_TIME_THRESHOLD_SECONDS
                        for s in schedules
                    ):
                        schedules.append(
                            ExtractedScheduleItem(
...
def _parse_schedule_time(text: str, base_time: datetime) -> datetime | None:
    ...
    for pattern_config in time_patterns:
        ...
        if match:
            ...
            # 处理"下午"、"晚上"等时间
            if hour_offset > 0 and hour < MAX_HOUR_FOR_TIME_PARSING:
                hour += hour_offset
