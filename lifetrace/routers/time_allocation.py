...
async def get_time_allocation(
    ...
    try:
        if start_date and end_date:
            ...
            stats_data = deps.db_manager.get_app_usage_stats(start_date=start_dt, end_date=end_dt)
        else:
            use_days = days if days else 7
            stats_data = deps.db_manager.get_app_usage_stats(days=use_days)
        ...
        def categorize_app(app_name: str) -> str:
            ...
            social_keywords = [
                'qq', 'wechat', 'weixin', '微信', 'telegram', 'discord', 'slack',
                'dingtalk', '钉钉', 'wxwork', '企业微信', 'feishu', '飞书', 'lark',
                'whatsapp', 'line', 'skype', 'zoom', 'teams', '腾讯会议'
            ]
            if any(keyword in app_lower for keyword in social_keywords):
                return "社交"
            
            # 浏览器
            browser_keywords = ['chrome', 'msedge', 'edge', 'firefox', 'browser', '浏览器', 'safari', 'opera', 'brave']
            if any(keyword in app_lower for keyword in browser_keywords):
                return "浏览器"
            
            # 开发工具
            dev_keywords = [
                'code', 'vscode', 'visual studio code', 'pycharm', 'idea', 'intellij',
                'webstorm', 'editor', '开发工具', 'sublime', 'atom', 'vim', 'neovim',
                'github desktop', 'git', 'github', 'gitkraken', 'sourcetree'
            ]
            ...
            # Office 办公软件
            office_keywords = ['word', 'excel', 'powerpoint', 'wps', 'libreoffice', 'office', 'onenote', 'outlook']
        ...
        deps.logger.error(f"获取时间分配数据失败: {e}")
        raise HTTPException(status_code=500, detail=f"获取时间分配数据失败: {str(e)}")
