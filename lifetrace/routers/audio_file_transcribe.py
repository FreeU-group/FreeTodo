...
def optimize_text_with_llm(text: str) -> str | None:
    """使用LLM优化转录文本"""
    try:
        ...
        system_prompt = get_prompt(
            "text_optimization",
            "system_assistant",
            default="你是一个文本优化助手，负责优化和整理转录文本，使其更加清晰、易读。",
        )
        user_prompt = get_prompt(
            "text_optimization",
            "user_prompt",
            text_content=text,
            default=f"请优化以下转录文本，使其更加清晰、易读：\n\n{text}",
        )
...
async def transcribe_file(
    ...
    try:
        ...
        logger.info(
            f"🎤 [分段转录] 开始上传文件: {file.filename}, 大小: {file.size if hasattr(file, 'size') else 'unknown'}"
        )
        ...
        logger.info(
            f"✅ [分段转录] 文件上传成功: {temp_file_path}, 大小: {file_size} bytes, 文件名: {file.filename}"
        )
        ...
        if file_ext in SUPPORTED_VIDEO_FORMATS:
            ...
            try:
                ...
                if result.returncode != 0:
                    ...
                    raise HTTPException(
                        status_code=500, detail="无法提取视频音频轨道，请确保已安装 ffmpeg"
                    )
                ...
                raise HTTPException(status_code=500, detail=f"提取视频音频失败: {str(e)}") from e
        ...
        try:
            ...
            logger.info(
                f"✅ [分段转录] 转录完成: {file.filename}, 文本长度: {len(transcript)} 字符"
            )
            ...
            logger.info(
                f"转录完成，文本长度: {len(transcript)}, 语言: {info.language}, 概率: {info.language_probability:.2f}"
            )

        except Exception as e:
            logger.error(f"转录失败: {e}", exc_info=True)
            raise HTTPException(status_code=500, detail=f"转录失败: {str(e)}") from e

        if not transcript or not transcript.strip():
            raise HTTPException(
                status_code=400, detail="转录结果为空，可能是音频文件无效或没有语音内容"
            )
                ...
                try:
                    ...
                    for todo_item in todo_items:
                        try:
                            ...
                            logger.info(
                                f"✅ 从文件转录自动创建Todo: {todo_item.title} (ID: {created_todo.id})"
                            )
                        except Exception as e:
                            logger.error(
                                f"创建Todo失败: {todo_item.title}, 错误: {e}", exc_info=True
                            )
                    ...
                    logger.info(
                        f"提取到 {len(todos)} 个待办事项，已创建 {len(created_todo_ids)} 个Todo"
                    )
        ...
        if extract_schedules:
            ...
            try:
                ...
                try:
                    ...
                    from lifetrace.routers.audio_schedule_extraction import (
                        _extract_schedules_with_llm,
                    )

                    reference_time = datetime.now()
                    schedule_items = await _extract_schedules_with_llm(
                        text_for_extraction, reference_time
                    )
